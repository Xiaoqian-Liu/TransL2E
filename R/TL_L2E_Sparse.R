#' Transfer Learning for Robust L2E Sparse Regression
#'
#' \code{TL_L2E_sparse} performs robust transfer learning by detecting 
...
#' @param penalty Penalty for sparsity (e.g., "lasso", "MCP", "SCAD").
...
#'   refine the sparse regression estimates.
...
#'                       
#'                       
#' TL_with_debias = TL_L2E_sparse(y_t, X_t, source_list = source_data,
...
#' 
TL_L2E_sparse <- function(y_t, X_t, source_list, 
                          nfolds = 5, penalty = "lasso", 
  ...
  loop_results <- foreach(i = seq_along(source_list),
    ...
    l2e_cur <- CV_L2E_sparse_ncv(y_cur, X_cur, penalty = penalty, 
    ...
    l2e_cur_opt <- L2E_sparse_ncv(y_cur, X_cur, lambdaSeq = lambda_val, 
  ...
  l2e_cl <- CV_L2E_sparse_ncv(y_fuse, X_fuse, penalty = penalty, 
  ...
  l2e_cl_opt <- L2E_sparse_ncv(y_fuse, X_fuse, lambdaSeq = lambda_colearn, penalty = penalty,
                               Show.Time = FALSE)
  ...
  if(debias){
    ...
    l2e_db <- CV_L2E_sparse_ncv(res_y, X_t, penalty = penalty, 
                                max_iter = max_iter, tol = tol, nfolds = nfolds, trace = FALSE)
    ...
    l2e_db_opt <- L2E_sparse_ncv(res_y, X_t, lambdaSeq = lambda_debias, penalty = penalty,
                                 Show.Time = FALSE)
